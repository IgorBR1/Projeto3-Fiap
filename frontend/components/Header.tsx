  ...
  return (
    <header className="sticky top-0 z-50 flex justify-between items-center px-6 py-4 bg-white border-b border-zinc-200 shadow-sm backdrop-blur-md bg-white/80 dark:bg-zinc-950/80 dark:border-zinc-800">
      ...
      {user ? (
          ...
          {user.role === "ADMIN" && (
            <Link className="text-sm font-medium text-zinc-600 hover:text-indigo-600 transition-colors dark:text-zinc-400 dark:hover:text-indigo-400"
             href="/admin" >
              Admin
            </Link>
          )}
        ...
        <div className="flex gap-4">
          ...
          <Link             className="text-sm font-medium text-zinc-600 hover:text-zinc-900 hover:bg-zinc-100 px-4 py-2 rounded-lg transition-all dark:text-zinc-400 dark:hover:text-zinc-100 dark:hover:bg-zinc-800"
 href="/login">
            Login
          </Link>

          <Link             className="text-sm font-medium bg-indigo-600 text-white hover:bg-indigo-700 px-4 py-2 rounded-lg shadow-sm transition-all focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-zinc-950"
 href="/register">
            Registrar
          </Link>
        </div>
